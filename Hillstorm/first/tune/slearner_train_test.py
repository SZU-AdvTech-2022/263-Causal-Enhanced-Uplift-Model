...
class SLearner(nn.Module):
    """
    S-Learner class -- single learner, treatment indicator just another feature
    """
    def __init__(self, input_dim, h_dim, is_self, act_type='elu'):
        super(SLearner, self).__init__()
        self.is_self = is_self

        self.s_fc0 = nn.Linear(input_dim + 1, h_dim)
...
def objective(trial):
    ...
    # trial:20
    # 0.019553786933224824 and parameters: {'rank': 64, 'lambda': 0.1, 'batch_size': 256, 'learning_rate': 0.0005}.
    # epoch: 0
    # loss: 56.41283
    # avg_loss: 0.42099
    # u_at_k: 0.06463
    ...
    # parameter settings
    train_file = "../train_Hillstrom.npy"
    valid_file = "../test_Hillstrom.npy"

    # model
    model = SLearner(input_dim=8, h_dim=rank, is_self=False, act_type="elu")
