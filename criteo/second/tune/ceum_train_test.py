...
class Ceum(nn.Module):
    def __init__(self, input_dim, input_dim1, h_dim, is_self, act_type='relu', margin=0.5, margin1=0.5, alpha=0.1,
        ...
        self.s_fc0 = nn.Linear(input_dim1 + 1, h_dim)
    ...
    def forward(self, feature_list, is_treat, feature_list1, is_treat1):
        ...
        s_last1 = self.act(self.s_fc4(self.act(self.s_fc3(self.act(self.s_fc2(self.act(self.s_fc0(xt1))))))))
        if self.is_self:
        ...
        _s_last1 = self.act(self.s_fc4(self.act(self.s_fc3(self.act(self.s_fc2(self.act(self.s_fc0(_xt1))))))))
        if self.is_self:
...
def collote_fn(batch_samples):
    ...
    for sample in batch_samples:
        ...
        batch_input1.append(sample[14:26])
        batch_is_treatment1.append(sample[26])
        batch_label1.append(sample[27])
...
def objective(trial):
    ...
    # 0.007655323437473259 and parameters: {'rank': 64, 'lambda': 0.0001, 'batch_size': 2048, 'learning_rate': 0.0001,
    #                                       'margin': 0.5, 'margin1': 0.8, 'alpha': 0.2, 'beta': 0.8}
    # trial:100
    # epoch: 3
    # loss: -12.11491
    # avg_loss: -0.29549
    ...
    #trial:80
    # 0.007655323437473259 and parameters: {'rank': 64, 'lambda': 0.0001, 'batch_size': 2048, 'learning_rate': 0.0001, 'margin': 0.5, 'margin1': 0.8, 'alpha': 0.2, 'beta': 0.8}
    # epoch: 3
    # loss: -12.11491
    # avg_loss: -0.29549
    # u_at_k: 0.00389
    ...
    # lamb = trial.suggest_categorical('lambda', [1e-5])
    # batch_size = trial.suggest_categorical('batch_size', [2048])
    # learning_rate = trial.suggest_categorical('learning_rate', [0.05])
    # margin = trial.suggest_categorical('margin', [0.9])
    # margin1 = trial.suggest_categorical('margin1', [0.9])
    # alpha = trial.suggest_categorical('alpha', [0.3])
    # beta = trial.suggest_categorical('beta', [0.2])

    setup_seed(seed)

    contrastive_pair_set = CRITEO('../contrastive_pair_criteo.npy')
    train_dataloader = DataLoader(contrastive_pair_set, batch_size=batch_size, collate_fn=collote_fn)

    test_data_set = CRITEO('../test_criteo.npy')
    valid_dataloader = DataLoader(test_data_set, batch_size=2048, collate_fn=collote_fn1)

    # model
    model = Ceum(input_dim=12, input_dim1=12, h_dim=rank, is_self=False, act_type="relu", margin=margin,
